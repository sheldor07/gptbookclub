...
# read json file
chapters = {}
with open(f'/Users/yajatgulati/dev/gptbookclub-dsai/BookData/ChapterJson/{book_name}.json') as json_file:
...
def generate_book_text():
    ...
    with open(book_path, 'rb') as pdf_file:
        ...
        chapter_begin_pages = [chapter_number[1]
        ...
        for page_num in range(num_pages):
            ...
            if page_num in chapter_begin_pages:
                ...
                curr_chapter = 'chapter {}'.format(
                    chapter_begin_pages.index(page_num) + 1)
                curr_chapter_text = []
    ...
    for _, chapter_data in data.items():
        ...
        for i, chunk in enumerate(chapter_text):
            ...
            content = get_response(
                user_prompt="Clean the following paragraphs and make it readable without changing the content. Only correct spelling and punctuation: " + chunk)
            ...
            pd_data['Embedding Vector'].append(
                get_embedding(content, embedding_model))

            time_taken = time.time() - start_time
            print('Performed cleaning of chunk', (i + 1), '/',
                  len(chapter_text), 'in', chapter_name, 'in', time_taken, 'seconds')
...
def generate_book_clusters():
    ...
    for chapter in chapters:
        ...
        for i in range(n_clusters):
            ...
            print('In cluster: ', i, '/', n_clusters, ',of chapter: ',
                  chapter_no, ' of ', len(chapters), ' chapters.')

            number_of_chunks = min(3, np.bincount(kmeans.labels_)[i])
            chunks = chapter_data.loc[chapter_data['Cluster'] == i, 'Content'].sample(
                number_of_chunks, random_state=42)
        ...
        print('Done with chapter clusters of chapter: ',
              chapter_no, ' of ', len(chapters), ' chapters.')
        chapter_no += 1
...
def generate_chapter_summaries():
    ...
    for cluster in chapter_clusters:
        ...
        print('Done with chapter summary of chapter: ', len(
            summaries['Summaries']), ' of ', len(chapter_clusters), ' chapters.')
...
def generate_book_clusters():
    ...
    matrix = np.vstack(df['Embedding Vector'].values)
    ...
    # Find n_cluster clusters for the entire book
    kmeans.fit(matrix)

    # Add a cluster label to the data
    labels = kmeans.labels_
    df["Cluster"] = labels
    ...
    for i in range(n_clusters):
        ...
        print('In cluster: ', i, '/', n_clusters)

        number_of_chunks = min(10, np.bincount(kmeans.labels_)[i])
        chunks = df.loc[df['Cluster'] == i, 'Content'].sample(
            number_of_chunks, random_state=42)
        chunks = ' '.join(chunks.values)

        response = get_response(
            system_prompt=f"You are the most well read agent on the book {book_name}",
            user_prompt="You are tasked with creating an amazing summary of concepts in the book. To assist you in this task, I will give you paragraphs from the book that are similar in nature. I need you to condense all that information into about 100 words. This information should be the important takeaways which could include things like ideas and concepts. It should, however, not include things such as long drawn explanations, descriptions or anything irrelevant. Your tone should be fun, helpful, witty and likeable. Do not sound like a sage. Write in a way that draws people to read more." + "\n\nParagraphs:\n\n" + chunks
        )

        cluster_summary.append(response)

    # Save the cluster summary for the book
    with open(clusters_path, 'w') as f:
        json.dump(cluster_summary, f)

    print('Saved clusters for the entire book')


# Generate the book overview
generate_book_clusters()
